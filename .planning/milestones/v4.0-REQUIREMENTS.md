# Requirements (ARCHIVED) — v4.0 Seven-Elements Conversational Architecture

**Milestone:** v4.0
**Status:** ✅ SHIPPED 2026-06-26 — all 16 requirements validated
...
> This is the milestone-close snapshot. The active REQUIREMENTS.md was removed at
> close; the next milestone starts fresh via `/gsd-new-milestone`.

---

## Organizational Context (ORG) — 3 requirements

- [x] **ORG-01** — User can provide organizational context through a 4-part Socratic step (work stream, organizational placement, reporting relationship, additional context); responses are assembled into `org_context: Optional[str]` on WorkDescription; `WDPatchRequest` updated in the same commit
- [x] **ORG-02** — Organizational context renders in the document live preview above the Client Service Results section
- [x] **ORG-03** — Organizational context populates the Part 2 Organizational Context section of the Accessible JD DOCX export
...
| MGR-01 | Phase 28 | ✅ Complete (Plan 28-01 — RoleSelector + userRole localStorage hydration + user_role drop guard) |
| MGR-02 | Phase 28 | ✅ Complete (Plan 28-02 — ClassifyBadge / Classification Sec / ReviewState audit panel suppression + 3 inspection tests) |
...
| SEXP-01 | Phase 29 | ✅ Complete (Plan 29-02 — export/json + build_seven_elements shared helper + manager-track placeholder) |
| SEXP-02 | Phase 29 | ✅ Complete (Plan 29-02 — export/csv + utf-8-sig BOM + one row per duty + per-element status) |
| SEXP-03 | Phase 29 | ✅ Complete (Plan 29-03 — Export JSON + CSV buttons + 4-branch exportAs() dispatch + OG guard bypass) |
| POST-01 | Phase 29 | ✅ Complete (Plan 29-02 — poster "About the Organization" section + build_poster_template.py self-verify) |
