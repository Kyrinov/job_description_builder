...
- **Phase 22 (SJD Library) complete 2026-06-11** — 4 plans: Wave 0 RED test scaffold (22-01), SJD_LIBRARY constant parsed from `data/SJD Examples.txt` (10 entries, OG code normalized) + `GET /api/sjd` + `GET /api/sjd/{number}` (22-02), `DraftDuty.source='sjd'` + `WorkDescription.sjd_source` + `POST /api/wd/{id}/sjd-start` + SJD manifest entry in `_build_v2_manifest` (22-03), `fetchSjds`/`fetchSjdDetail` helpers + Browse SJDs modal panel + SJD-03 og_code-change warning toast + SJD provenance badge in document preview (22-04). 125 backend + 60 frontend tests GREEN. Code review: 1 critical (CR-01 latent parser crash on malformed input, current data clean) + 8 warnings + 7 info, all advisory. 4 human-browser-verification items persisted in 22-HUMAN-UAT.md (SJD-03 toast, SJD badge render, Browse SJDs UX, og_level-only no-toast). Requirements SJD-01/02/03 validated.
- **Phase 23 (Writing Guide Integration) complete 2026-06-15** — 4 plans: Wave 0 RED baseline + duty_validator.py stub (23-01), 4-rule structural duty validation (active-voice, 8–25 word count, no-passive, no-duplicate) (23-02), `POST /api/wd/{id}/validate-duties` endpoint (23-03), frontend `dutyHints` state + `client_service_results` step + `OG_DUTY_TIPS` per-group tips + `.duty-hint`/`.og-duty-tip` CSS (23-04). 134 backend + 60 frontend tests GREEN. Code review clean. 4 human-browser-verification items in 23-HUMAN-UAT.md (duty-hint rendering, OG tips, validate-duties trigger, client service results step). Requirements WG-01/02/03/04 validated.
...
- **Phase 26 (Org Context Conversational Step) complete 2026-06-24** — 2 plans: Wave 0 RED baseline (26-01, 8 stubs across 5 test files) + Wave 1 GREEN implementation (26-02). New `org_context: Optional[str]` typed root field on `WorkDescription` + `WDPatchRequest.org_context` (max_length=4000, ASVS V5) in same commit (co-update rule). `stepIndex` resume-by-last-answered lazy initializer (replaces `useState(0)`) landed BEFORE the STEPS insertion so existing sessions survive STEPS growth. `OrgContextInput` component (4 sub-fields → assembled string) + `org_context` step in STEPS before `client_service_results`. `org_context` + `client_service_results` Secs in DocumentPane with dynamic `n++` numbering. `export_service._build_wd_context` prefers `wd.org_context` over synthesized `_build_organizational_context_text()` fallback. 153 backend + 65 frontend tests GREEN. Code review found 1 Critical (CR-01: `commit()` mirror list omitted `org_context` → silent data-loss on SPA→export; fixed in 02fb8d6) + 1 Warning (WR-01: OrgContextInput ignores value prop on re-edit — deferred UX) + 4 info. Verification passed 5/5 must-haves (CR-01 fix traced through real SPA→export code path). Requirements ORG-01/02/03 validated.
- **Phase 27 (Responsibilities Narrative + Completeness Audit) complete 2026-06-24** — 2 plans: Plan 01 (Wave 1, RESP vertical slice mirroring Phase 26) + Plan 02 (Wave 2, ELEM completeness audit). New `responsibilities_narrative: Optional[str]` typed root field on `WorkDescription` + `WDPatchRequest.responsibilities_narrative` (max_length=4000, ASVS V5, co-update rule) in same commit. Single free-text textarea step in STEPS after `duties`, before `quals` — no new component (R-RESP-01: simpler than org_context's 4-part assembly). DocumentPane conditional Responsibilities Sec with dynamic `n++` (RESP-02). `export_service._build_wd_context` responsibilities_text = `(wd.responsibilities_narrative or "").strip() or _ADVISOR_PLACEHOLDER` — REPLACES the previous JES-derived factor list (RESP-03, R-RESP-03; no existing test asserted the JES variant). `build_seven_elements(wd)` shared helper returning 7 elements + `complete_count` + `total=7` with status rules: typed-field-only audit for Organizational Context (ROADMAP #4); never `not_applicable` for Responsibility (ROADMAP #3); `derived` from `wd.jes_total_points is not None` for Effort/Working Conditions. `POST /api/wd/{id}/validate-elements` endpoint (404 guard). Review-phase N/7 completeness badge as soft gate (ROADMAP #5: export buttons stay enabled at any count). 172 backend + 70 frontend tests GREEN. Code review clean (0 critical, 0 warning, 3 info — all pre-existing patterns preserved). Verification 11/11 must-haves passed. Requirements RESP-01/02/03 + ELEM-01/02/03 validated. ELEM-03 literal JSON/CSV routes are Phase 29 (SEXP-01/02 per CONTEXT R-ELEM-03).
...
---
*Last updated: 2026-06-24 — Phase 27 (Responsibilities Narrative + Completeness Audit) complete; RESP-01/02/03 + ELEM-01/02/03 validated. v4.0 milestone 2/4 phases done (26–29). 172 backend + 70 frontend tests GREEN.*
